...
class FileTransferBot {
    ...
    // Helper to run FFmpeg and strictly capture format/codec errors
    runFFmpeg(args) {
        return new Promise((resolve, reject) => {
            ...
            ffmpeg.on('close', code => {
                if (code === 0) {
                    ...
                    // Extract the last chunk of the log for debugging
    ...
    async start() {
        ...
        try {
            ...
            // 2. Video Normalization & Extension Fix (Path 1: MP4 Standardization)
            if (isVideo) {
                // Force filename to end with .mp4 for Rubika/Bale proper recognition
                ...
                try {
                    if (shouldCompress) {
                        ...
                        // Remux to MP4 safely without losing quality
                        await this.runFFmpeg([
                            '-i', downloadedFilePath,
                            '-c', 'copy',
                            '-movflags', '+faststart',
